...
// removed '/with/key/' + key + from url

var homeassistant_url = 'https://28614nf.duckdns.org/api/webhook/nightscout'
...
function init (env) {
  ...
  maker.makeKeyRequest = function makeKeyRequest(key, event, eventName, callback) {
    var url = homeassistant_url + eventName + maker.valuesToQuery(event);
    request
      .get(url)
